...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item Known number of landmarks (This is not a constraint but its implementation need some checks in the algorithm: check if the state vector is full and so some bookkeeping). It will drag away the attention to the important concepts.
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \note{We can limit the use of landmark by their index. Each time the landmark a new landmark is observed we increase the index so that landmark is incorporated. We do this until we reach the maximum of landmarks that we can work with.}
