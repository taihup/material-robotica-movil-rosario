...
\begin{frame}
    ...
    \begin{algorithmic}[1]
        \Procedure{EKFSLAMPrediction}{$\mu_{t-1}, \Sigma_{t-1}, u_t, z_t, c_t, R_t$}
        ...
        \end{bmatrix} F_x$
        \vspace{1em}
        \State $\bar{\Sigma}_t = G_t \Sigma_{t-1} G_t^T + \underbrace{F_x^T R_t F_x}_{R_t}$
        \EndProcedure
    \end{algorithmic}
...
\begin{frame}
    ...
    \begin{equation*}
        ...
        \bar{\mu}_{j,x} \\
        \bar{\mu}_{j,y}
        \end{bmatrix}}_{\text{location of landmark } j}
        =
        \underbrace{
        \begin{bmatrix}
        \bar{\mu}_{t,x} \\
        \bar{\mu}_{t,y}
        \end{bmatrix}}_{\text{estimated location of the platform}}
        +
        \underbrace{
        \begin{bmatrix}
...
\begin{frame}
    ...
    \begin{algorithmic}[1]
        \State $Q_t \gets \begin{pmatrix} \sigma_r^2 & 0 \\ 0 & \sigma_\theta^2 \end{pmatrix}$ \Comment{Measurement noise covariance}
        \For{all observed features $z_i^t = (r_i^t, \phi_i^t)^T$}
            \State $j \gets c_i^t$ \Comment{Landmark identifier}
            \If{landmark $j$ never seen before}
                \State $\begin{pmatrix} \bar{\mu}_{j,x} \\ \bar{\mu}_{j,y} \end{pmatrix} \gets 
                       \begin{pmatrix} \bar{\mu}_{t,x} \\ \bar{\mu}_{t,y} \end{pmatrix} + 
                       \begin{pmatrix} r_i^t \cos(\phi_i^t + \bar{\mu}_{t,\theta}) \\ r_i^t \sin(\phi_i^t + \bar{\mu}_{t,\theta}) \end{pmatrix}$
                \Comment{Initialize new landmark position}
            \EndIf
            \State $\delta \gets \begin{pmatrix} \delta_x \\ \delta_y \end{pmatrix} = 
                   \begin{pmatrix} \bar{\mu}_{j,x} - \bar{\mu}_{t,x} \\ \bar{\mu}_{j,y} - \bar{\mu}_{t,y} \end{pmatrix}$
            \State $q \gets \delta^T \delta$
            \State $\hat{z}_i^t \gets \begin{pmatrix} \sqrt{q} \\ \operatorname{atan2}(\delta_y, \delta_x) - \bar{\mu}_{t,\theta} \end{pmatrix}$
...
\begin{frame}
    ...
    \begin{algorithmic}[1]
        ...
        \For{all observed features $z_i^t = (r_i^t, \phi_i^t)^T$}
            ...
            0 & 0 & 0 & 0 & \cdots & 0 & 0 & 1 & \cdots & 0
            \end{pmatrix}$ \Comment{Jacobian of state mapping}
            
            \State $H_t^i \gets \frac{1}{q} \begin{pmatrix}
            -\sqrt{q}\delta_x & -\sqrt{q}\delta_y & 0 & \sqrt{q}\delta_x & \sqrt{q}\delta_y \\
            \delta_y & -\delta_x & -q & -\delta_y & \delta_x
            \end{pmatrix} F_{x,j}$ \Comment{Measurement Jacobian}
            
            \State $K_t^i \gets \Sigma_t H_t^{iT}(H_t^i \Sigma_t H_t^{iT} + Q_t)^{-1}$ \Comment{Kalman gain}
            \State $\bar{\mu}_t \gets \bar{\mu}_t + K_t^i(z_t^i - \hat{z}_t^i)$ \Comment{State update}
            \State $\Sigma_t \gets (I - K_t^i H_t^i) \Sigma_t$ \Comment{Covariance update}
        \EndFor
        \State $\mu_t \gets \bar{\mu}_t$ \Comment{Final state estimate}
        \State $\Sigma_t \gets \bar{\Sigma}_t$ \Comment{Final covariance estimate}
