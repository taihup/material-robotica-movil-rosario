...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \]
        \[
        \hat{z}^i_t = \begin{pmatrix}
            \sqrt{q} \\
            \text{atan2}(\delta_y, \delta_x) - \bar{\mu}_{t,\theta}
        \end{pmatrix}
        \]
    
        \item \textbf{Compute the Jacobian}
        \[
        {}^{\text{low}}H^i_t = \frac{\partial h(\bar{\mu}_t)}{\partial \bar{\mu}_t}
        \]
        \[
        = \frac{1}{q}
        \begin{pmatrix}
            -\sqrt{q} \delta_x / \delta_y & -\sqrt{q} \delta_y / \delta_x & 0 \\
            +\sqrt{q} \delta_x / -\delta_y & \sqrt{q} \delta_y / \delta_x & -q
        \end{pmatrix}
        \]
    \end{itemize}
...
\begin{frame}
    ...
    \begin{itemize}
        \item \textbf{Use the computed Jacobian}
        \[
        {}^{\text{low}}H^i_t = \frac{1}{q}
        \begin{pmatrix}
            -\sqrt{q} \delta_x / \delta_y & -\sqrt{q} \delta_y / \delta_x & 0 \\
            +\sqrt{q} \delta_x / -\delta_y & \sqrt{q} \delta_y / \delta_x & -q
        \end{pmatrix}
        ...
        \item \textbf{Map it to the high dimensional space}
        \[
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item Compute the Jacobian
        \begin{equation*}
            \text{low } H_t^i = \frac{1}{q} \begin{bmatrix}
...
\begin{frame}
    ...
    % \begin{algorithmic}[1]
    % \State $Q_t = \begin{bmatrix} \sigma_r^2 & 0 \\ 0 & \sigma_\phi^2 \end{bmatrix}$
    % \For{all observed features $z_t^i = (r_t^i, \phi_t^i)^T$}
    % \State $j = c_t^i$
    % \If{landmark $j$ never seen before}
    % \State Initialize $\begin{bmatrix} \mu_{j,x} \\ \mu_{j,y} \end{bmatrix}$
    % \EndIf
    % \State Compute $\delta$ and $q = \delta^T \delta$
    % \State $\hat{z}_t^i = \begin{bmatrix} \sqrt{q} \\ \text{atan2}(\delta_y, \delta_x) - \mu_{t,\theta} \end{bmatrix}$
    % \end{algorithmic}
\end{frame}
