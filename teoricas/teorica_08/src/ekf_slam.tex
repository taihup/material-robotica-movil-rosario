...
\begin{frame}
    ...
    \begin{algorithmic}[1]
        ...
        \State $Q_t = \begin{pmatrix} \sigma_r^2 & 0 \\ 0 & \sigma_\theta^2 \end{pmatrix}$ \Comment{Measurement noise covariance}
        \For {all observed features $z_i^t = (r_i^t, \phi_i^t)^{\top}$}
            \State $j = c_i^t$ \Comment{Landmark identifier}
            \If{landmark $j$ never seen before}
                \State $\begin{pmatrix} \bar{\mu}_{j,x} \\ \bar{\mu}_{j,y} \end{pmatrix} = 
            ...
            \EndIf
            \State $\delta = \begin{pmatrix} \delta_x \\ \delta_y \end{pmatrix} = 
                   \begin{pmatrix} \bar{\mu}_{j,x} - \bar{\mu}_{t,x} \\ \bar{\mu}_{j,y} - \bar{\mu}_{t,y} \end{pmatrix}$
            \State $q = \delta^T \delta$
            \State $\hat{z}_i^t = \begin{pmatrix} \sqrt{q} \\ \operatorname{atan2}(\delta_y, \delta_x) - \bar{\mu}_{t,\theta} \end{pmatrix}$
...
\begin{frame}
    ...
    \begin{algorithmic}[1]
        ...
        \Statex
        \Statex
        \State $H_t^i = \frac{1}{q} \begin{pmatrix}
            -\sqrt{q} \delta_x & -\sqrt{q} \delta_y & 0 & \sqrt{q} \delta_x & \sqrt{q} \delta_y\\
            \delta_y & -\delta_x & -q & -\delta_y & \delta_x
