...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item \emph{Combinatorial Planning}:
            Caracteriza a $\configurationSpace$-space explícitamente al capturar el conectividad de $\configurationSpace$-space en un gráfico y encuentra soluciones usando la búsqueda
            \item Planificación basada en muestreo
            Utiliza detección de colisiones para sondear y
            buscar incrementalmente en el $\configurationSpace$-space una solución
        \end{itemize}
...
\begin{frame}
    ...
    \begin{itemize}
        \item     El problema de búsqueda: encontrar una secuencia.
        de acciones (un camino) que conduce a lo deseable
        estados (una meta)

        \begin{itemize}
            \item Búsqueda desinformada: además del problema definición, no hay más información sobre el dominio (``búsqueda ciega'')
            \item Lo único que se puede hacer es ampliar nodos de manera diferente
            \item Algoritmos de ejemplo: amplitud primero, costo uniforme, profundidad primero, bidireccional, etc.
        \end{itemize}
