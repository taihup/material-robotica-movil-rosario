...
\begin{frame}
    ...
    \TODO{Agregar ejemplo de traslación como hice con la rotación}
\end{frame}
...
\begin{frame}
    ...
    Describe una rotación en relación con el sistema de coordenadas local del objeto. Los ejes del objeto pueden cambiar durante la rotación. Dado por ejemplo el orden de rotación xyz (primero rotar en $x$, luego en $y$ y luego en $z$), la matríz de rotación final $R$ se construye Posmultiplicación: $R = R_{x}(\alpha) \, R_{y}(\beta) \, R_{z}(\gamma)$.
    \end{block}
    ...
    \begin{block}{Rotación Extrínseca}
        Describe una rotación en relación con un sistema de coordenadas global fijo en el espacio. El sistema de referencia es externo al objeto que está siendo rotado. Dado por ejemplo el orden de rotación xyz, la matríz de Rotación final $R$ se construye Premultiplicación: $R = R_{z}(\gamma) \, R_{y}(\beta) \, R_{x}(\alpha)$.
    \end{block}
...
\begin{frame}
    ...
    La matríz de Rotación intrínseca obtenida por el orden de rotación $xy^{\prime}z^{\prime\prime}$ es igual a la matríz extrínseca obtenida en el orden $xyz$.
    
    \begin{equation*}
        \underbrace{R_{x}(\alpha) \, R_{y^{\prime}}(\beta) \, R_{z^{\prime\prime}}(\gamma)}_{\text{Rotación intrínseca}} = \underbrace{R_{z}(\gamma) \, R_{y}(\beta) \, R_{x}(\alpha)}_{\text{Rotación extrínseca}}
    \end{equation*}
    
    \begin{figure}[!h]
        \subfloat[$R = R_{x}(\SI{180}{\degree}) \, R_{y^{\prime}}(\SI{45}{\degree}) \, R_{z^{\prime\prime}}(\SI{90}{\degree})$]
        {
...
\begin{frame}
    ...
    \begin{figure}[!h]
        ...
        \subfloat[$R = R_{z}(\alpha) \, R_{x}(\beta) \, R_{z}(\gamma)$]
        {
