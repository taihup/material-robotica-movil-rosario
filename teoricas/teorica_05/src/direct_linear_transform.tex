...
\begin{frame}
    ...
    \end{bmatrix}
    \end{equation*}
    \vspace{-1.5cm} % Pull up the matrix slightly to make space for the note
    \begin{tikzpicture}[overlay, remember picture]
        \draw[->, thick, red] ([xshift=2cm, yshift=0.5cm]current bounding box.south) -- ([xshift=4cm, yshift=1cm]current bounding box.south) node[right, red, align=left] {e.g., assume all $Z_i=0$};
    \end{tikzpicture}
\end{frame}
...
\begin{frame}
    ...
    \vspace{0.5cm}
    \centering
    % Simplified matrix structure focusing on the blocks
    \begin{equation*}
    \mathbf{M} = 
    \begin{bmatrix}
    \cdots \\
    ...
    % Matrix showing the blocks with red boxes (using TikZ for the boxes)
    \vspace{0.5cm}
    \begin{tikzpicture}[overlay, remember picture]
    \node (M) at (0, 0) {
        $-\begin{bmatrix}
    ...
    % First Red Box (around -Z_i, -1, 0, 0, 0, 0) - Approximate
    \draw[red, ultra thick] (M.south west) ++(1.8cm, 0.45cm) rectangle ++(1.7cm, 0.9cm);
    % Second Red Box (around x_i X_i, x_i Y_i, y_i Z_i, y_i) - Approximate
    \draw[red, ultra thick] (M.south west) ++(5.7cm, 0.45cm) rectangle ++(1.7cm, 0.9cm);
    % Third Red Box (around y_i X_i, y_i Y_i, y_i Z_i, y_i) - Approximate
    \draw[red, ultra thick] (M.south west) ++(8.8cm, 0.45cm) rectangle ++(1.7cm, 0.9cm);

    % Arrow and Text
    \draw[->, red, ultra thick] (M.north east) ++(-2.5cm, 0.5cm) -- ++(0.5cm, 0) node[right, red, align=left] {\bfseries rank deficiency};
    \end{tikzpicture}
    \vspace{2cm} % Add space for the overlay to not clip
\end{frame}
