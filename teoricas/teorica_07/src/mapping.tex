...
\begin{frame}
    ...
    \note{Un \emph{forward model} predice las lecturas del sensor basándose en parámetros conocidos del sistema, mientras que un \emph{inverse model} estima los parámetros del sistema a partir de las lecturas observadas del sensor.}

    \note{Un modelo de sensor frontal, dado un mapa hipotético de cuadrícula de ocupación, predeciría las lecturas del sonar que el robot debería observar. Consideraría todas las celdas ocupadas dentro del cono del sonar. Por ejemplo, podría calcular la probabilidad de detectar el obstáculo más cercano, el segundo más cercano (si no se detecta el primero) o incluso una lectura aleatoria debido a un fallo del sensor o a una reflexión especular.}

	
    \note{Más info: forward measurement model vs inverse measurement model https://www.cs.cmu.edu/~thrun/papers/thrun.occ-journal.pdf} 

	\note{Más info: http://robots.stanford.edu/papers/thrun.iros01-occmap.pdf}
...
\begin{frame}
    \frametitle{Ejemplo: Occupancy Grid Map}
    \note{información extraída de https://youtu.be/v-Rm9TUG9LA}
