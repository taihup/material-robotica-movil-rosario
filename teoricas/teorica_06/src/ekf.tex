...
\begin{frame}
	...
	\begin{align*}
		p(\state_{t} | \controlCommand_{t}, \state_{t-1}) &\approx \det(2 \pi R_{t})^{\frac{1}{2}}\\
		&\exp (-\dfrac{1}{2} (\state_{t} - \motionModelFunction{\controlCommand_{t}-\mu_{t-1}} - G_{t}(\state_{t-1}-\mu_{t-1}))^{\top}\\
		&\inverse{R_{t}} (\state_{t} - \underbrace{\motionModelFunction{\controlCommand_{t},\mu_{t-1}} - G_{t} (\state_{t-1}-\mu_{t-1})}_{\text{modelo linearizado}}))
	\end{align*}
	
	\item $R_{t}$ describe el ruido del movimiento
	\end{itemize}	
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		\item Linearizar el modelo esta dado por:
		\begin{align*}
			p(\observation_{t} | \state_{t}) &\approx \det(2 \pi Q_{t})^{\frac{1}{2}}\\
			&\exp (-\dfrac{1}{2} (\observation_{t} - \observationModelFunction{\overline{\mu_{t}}} - H_{t}(\state_{t} - \overline{\mu_{t}}))^{\top}\\
			&\inverse{H_{t}} (\observation_{t} - \underbrace{\observationModelFunction{\overline{\mu_{t}}} - H_{t} (\state_{t}-\overline{\mu_{t}})}_{\text{modelo linearizado}}))
		\end{align*}
		
		\item $Q_{t}$ describe el ruido de la medición
	\end{itemize}	
