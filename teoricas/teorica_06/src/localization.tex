...
\begin{frame}
    ...
    \only<3>{\alert{Markov assumption. Lo que mido ahora solo depende del estado anterior y es independiente de las mediciones y los comandos anteriores.}}
    \only<4>{\alert{Ley de probabilidad total. Es decir, integramos sobre todos los estados anteriores posibles.}}
    \only<5>{\alert{Markov assumption.}}
    \only<6>{\alert{Independence assumption.}} \note{Perdemos información ya que ayumismos independencia. Asumismo que un comando futuro no nos ayuda en saber nuestra pose actual. Esto no siempre es cierto.}
