...
\end{lstlisting}
	...
	Implemente el algoritmo de Filtro de Kalman Extendido en \lstinline[style=bash]{ekf.py}. Deberá completar \lstinline[style=bash]{ExtendedKalmanFilter.update} y los métodos de campo $G$, $V$ y $H$. Sus resultados de una implementación exitosa de EKF deben ser comparables	a los siguientes resultados.
...
\end{lstlisting}
		...
		\item Trace el error de posición medio y ANEES (error de estimación normalizado promedio al cuadrado) como el filtro $\alpha$, $\beta$ los factores varían sobre $r$ (como arriba) mientras que los datos se generan con el valor predeterminado. Comentar cualquier cosa que sea de interes observar.
	\end{enumerate}


	\ejercicio Implementar PF
	
	Implemente el algoritmo de filtro de partículas en \lstinline[style=bash]{pf.py}. Deberá completar \lstinline[style=bash]{ParticleFilter.update} y \lstinline[style=bash]{ParticleFilter.resample}.
