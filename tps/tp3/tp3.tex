...
\begin{itemize}
    ...
    \item El uso de IA está estrictamente prohibido, siendo la nota máxima 6.
\end{itemize}

\section{Datos}
En este trabajo práctico se utilizará el dataset EuRoC\footnote{\url{https://projects.asl.ethz.ch/datasets/doku.php?id=kmavvisualinertialdatasets}}. Rosbags ya convertidos a formato ros2 pueden ser descargados de \url{http://fs01.cifasis-conicet.gov.ar:90/~pire/datasets/euroc_rosbag2/}. Observar que el dataset cuenta con \emph{ground-truth} de la trayectoria del robot en el sistema de coordenadas de la IMU.


%Para el ejercicio de calibración, debe convertir la secuencia de calibración a formato ROS2. Para esto puede utilizar el software \lstinline{rosbags}\footnote{https://pypi.org/project/rosbags/}.

%Los rosbags del dataset por defecto no cuentan con mensajes de \lstinline{camera_info} requeridos para rectificar las imágenes. Rosbags con mensajes de \lstinline{camera_info} pueden ser descargados de \url{http://fs01.cifasis-conicet.gov.ar:90/~pire/datasets/euroc_rosbag2/MH_01_easy_with_camera_info/}.


\section{Ejercicios}

\ejercicio Realizar una calibración de la cámara estéreo del robot. Se debe proveer los parámetros intrínsecos y extrínsecos. Analizar la calibración obtenida.
...
\end{lstlisting}
    ...
    Analizar los resultados obtenidos. Agregar capturas al informe.

\item \textbf{Extraer Feaures Visuales: Keypoints y descriptores}

Selecionar un detector de keypoints (FAST, ORB, SIFT, SURF, GFTT, BRISK, etc.) y un descriptor (BRIEF, ORB, BRISK, etc.), y extraer features en ambas imágenes. Capturar una imagen izquierda y derecha con los features extraídos. Analizar los resultados obtenidos y agregar capturas al informe.

\item \textbf{Buscar correspondencias visuales}

Realizar la búsqueda de correspondencias entre los feature de ambas imágenes (\emph{matching}). Para esto se debe utilizar la función \lstinline{cv::BFMatcher::BFMatcher()}. Visualizar todos los matches. Luego, visualizar todos los matches que tengan una distancia de matching menor a 30. Analizar los resultados obtenidos y agregar capturas al informe.

\item \textbf{Triangular Puntos 3D}

Dadas las correspondencias visuales (\emph{matches}) obtenidas en el paso anterior, realizar la triangulación de los features extraídos utilizando la función \lstinline{cv::sfm::triangulatePoints()}. Para la visualización de la nube de puntos 3D se puede publicar un mensaje de tipo \lstinline{sensor_msgs/PointCloud2}\footnote{\url{http://docs.ros.org/en/melodic/api/sensor_msgs/html/msg/PointCloud2.html}} y hacer uso de RViz. Agregar a cada punto 3D el color del pixel de la imagen izquierda.  Analizar los resultados obtenidos y agregar capturas al informe.

\item \textbf{Filtrar de Correspondencias Espúreas}

Aplicar RANSAC (\emph{Random sample consensus}) para filtrar los matches espúreos y computar la matríz fundamental que relaciona los puntos. Para esto puede utilizar la función \lstinline{cv::findFundamentalMat()}. Visualizar los matches entre las imágenes nuevamente como la nube de puntos 3D generada. También visualizar las líneas epipolares en la imagen izquiedan resultante de aplicar a cada pundo de la imagen derecha la matríz Fundamental. Para esto último utilizar la función  \lstinline{cv::computeCorrespondEpilines()}. Analizar los resultados obtenidos y agregar capturas al informe.

\item (Opcional) \textbf{Feature Mapping con Localización \emph{ground-truth}}

Mapear el entorno utilizando las poses dada por el \emph{ground-truth} del dataset. Agregar a cada punto 3D el color del pixel de la imagen izquierda. Visualizar el mapa reconstruido.  Analizar los resultados obtenidos y agregar capturas al informe.

\item \textbf{Computar el Mapa de Disparidad}

Computar el mapa de disparidad con las librerías utilizando la función \lstinline{cv::StereoMatcher::compute()}. Opcionalmente para tener mejores resultados puede utilizar la librería LIBELAS\footnote{\url{http://www.cvlibs.net/software/libelas/}}. Visualizar el mapa de disparidad. Analizar los resultados obtenidos y agregar capturas al informe.

\item \textbf{Reconstruir la Escena 3D de manera Densa}

Utilizando el mapa de disparidad obtenido en el paso anterior realizar una reconstrucción densa de la escena observada utilizando la función \lstinline{cv::reprojectImageTo3D()}. Para esto debe utilizar la matríz de reproyección Q retornada por la función \lstinline{cv::stereoRectify()}. Visualizar la nube de puntos 3D. Analizar los resultados obtenidos y agregar capturas al informe.

\item (Opcional) \textbf{Dense Mapping con Localización Ground-truth}

Mapear el entorno de manera densa utilizando las poses dada por el \emph{ground-truth} del dataset. Agregar a cada punto 3D el color del pixel de la imagen izquierda. Visualizar el mapa reconstruido.  Analizar los resultados obtenidos y agregar capturas al informe.
...
\begin{itemize}
    \item Visualizar la pose estimada de ambas cámaras. Analizar los resultados obtenidos y agregar capturas al informe.
    \item Estimar y visualizar la trayectoría realizada por la cámara izquierda utilizando como factor de escala la distancia entre cada par de frames dada por el \emph{ground-truth}. Analizar los resultados obtenidos y agregar capturas al informe.
\end{itemize}
