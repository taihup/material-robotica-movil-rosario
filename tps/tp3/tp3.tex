...
\TODO{Hacer que el driver de la TARA funcione correctamente y genere las imágenes izquierda, derecha y los camera\_info con los mimos timestamps.}

Dado que el rosbag tiene las imágenes izquierda y derecha desincronizadas (diferente timestamp) debido a un bug en el driver de la cámara. Vamos a tener que sincronizar los mensajes de las imágenes utilizar el paquete \lstinline{message_filters}\footnote{\url{http://wiki.ros.org/message_filters}}. Para esto hay que 
...
Utilizando \lstinline{cv::recoverPose()} estimar la transformación entre la imagen izquierda y la imagen derecha. Para esto deberá calcular la matríz esencial utilizando la función \lstinline{cv::findEssentialMat()}. Notar que \lstinline{cv::recoverPose()} retorna el vector unitario de traslación, por lo tanto deberá multiplicarlo por el factor de escala (en este caso el baseline) para obtener la traslación estimada. Una vez hecho esto se pide:


\begin{itemize}
    \item Visualizar la pose estimada de ambas cámaras.
    \item Estimar y visualizar la trayectoría realizada por la cámara izquierda a un factor de escala.
\end{itemize}
